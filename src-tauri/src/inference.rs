...
pub async fn create_translation_response(
    ...
    let translation = state.pipeline.as_mut().expect("pipeline should be initialized").run(&format!("TASK: Translate Latin sentences to English sentences.\n\nLatin: {}\nEnglish translation:", input_string.trim()), 140 * 3).map_err(|e| e.to_string())?;
    use ring::digest;
    ...
    loop {
        ...
        let first_32_bits = ((digest[0] as i32) << 16) | ((digest[1] as i32) << 8) | digest[2] as i32;
...
pub async fn download_model(window: tauri::Window) -> Result<PathBuf, String> {
    ...
    .await
    .or(Err(String::from("Failed to download model")))?;
    let total_size = res.content_length().ok_or(String::from(
        "Failed to fetch content-length when downloading",
    ))?;
